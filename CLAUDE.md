...
`docs/runtime-discipline.md` (hand-maintained) is this repo's control-stage rulebook. All general Factorio 2.x prototype/API reference lives in the global factorio-modding skill (see below) — do not rely on training data for field names or API signatures. Where any doc names a third-party mod, that is an evidence citation — a convergent pattern observed during research — never code provenance; no third-party code or assets are copied (see runtime-discipline.md §11).
...
- The Reef is a `space-location`, not a `planet` — verify field differences in the global skill's `prototype-index.md` before writing prototype definitions
- Check the global skill's `breaking-changes-2x.md` before using any API method or prototype field that looks familiar from 1.x
...
- One feature branch per system (e.g. `ASTEROIDS`), branched from `main`; merge to `main` via PR when the system is complete. Never commit directly to `main`. (`BUILD-1` is the closed-out general branch — no new commits there.)
