...
- **`type_ratios` must have control points spanning the full range of every position used anywhere in the route table** (its own points plus every `probability_on_range_*` point) — at least one `type_ratios` point at-or-before the earliest such position and one at-or-after the latest. `find_lower_point`/`find_higher_point` (in vanilla's `asteroid-spawn-definitions.lua`) return an empty default (`{position = 0}` or `{position = 1}`, neither has a `.ratios` field) whenever no real control point brackets the queried position on that side. Two concrete traps this produces, both confirmed via load-time crashes:
  - A control point placed at exactly `position = 0.0`: `find_lower_point` seeds its search at `{position = 0}` and only replaces it on a *strict* `data_point.position > position_data.position` — a point at exactly `0.0` can never beat that default, so the lower bound silently falls through empty. Crashes as `pairs(nil)` inside `interpolate_point_ratio` (reading `pointA.ratios`).
  - A `type_ratios` table with too few points to reach a later `probability_on_range_*` position (e.g. only one point, or its last point short of that position): `find_higher_point` can't find anything at-or-beyond the queried position, so the upper bound falls through empty instead. Crashes as `pairs(nil)` inside `normalize_ratio` (reading `pointB.ratios`), one level deeper in the same call chain (`interpolate_point_ratio` → `normalize_ratio`).
  Vanilla's own route tables avoid both: their earliest control point is never `0.0` (always `0.1`+), and `type_ratios` always spans at least as wide as every `probability_on_range_*` table in the same route (e.g. `fulgora_aquilo`'s `type_ratios` covers `0.1`–`0.9`, matching its `probability_on_range_*` spans).
